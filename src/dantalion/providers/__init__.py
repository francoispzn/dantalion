...
_ALIASES = {"openai-compatible": "openai", "vllm": "openai", "lmstudio": "openai"}


def build_provider(kind: str, model: str, **kwargs: Any) -> Provider:
    """Construct a provider by name.

    ``kind`` selects the adapter (``ollama``, ``openai``, ``mock``); ``kwargs``
    are passed through, so e.g. ``base_url`` reaches the adapter unchanged.
    Common synonyms (``vllm``, ``lmstudio``, ``openai-compatible``) map to the
